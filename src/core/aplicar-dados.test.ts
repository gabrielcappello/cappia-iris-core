...
// --- CAS ESTRITO da transicao pos-conclusao (docs/07-arquitetura-v2.md
// secao 10, Etapa 2). Prova a propriedade compartilhada pelos TRES desfechos
// concluintes: `reserva_criada`, `remarcacao_criada` e `cancelamento_criado`
// atravessam este mesmo caminho, com a mesma limpeza e a mesma publicacao. ---

const BASE_DA_DECISAO = new Date('2026-07-01T00:00:00.000Z').toISOString();
const ESTADO_JA_AVANCADO = new Date('2026-07-01T00:00:05.000Z').toISOString();

for (const marcador of ['reserva_criada', 'remarcacao_criada', 'cancelamento_criado'] as const) {
  test(`cas_estrito (${marcador}): estado ja avancado -- NAO escreve, NAO rele, NAO reaplica`, async () => {
    const tabelas = criarTabelasFalsasVazias();
    // Estado JA AVANCADO por um turno posterior, com campos que a limpeza
    // removeria se fosse reaplicada sobre ele.
    const conversa = semearEstado(
      tabelas,
      { periodo: 'manha', procedimento_id: 'do-turno-b' },
      { atualizado_em: ESTADO_JA_AVANCADO }
    );
    const cliente = new ClienteFalso(tabelas);
    ...
    const resultado = await aplicarDados(cliente, {
      ...
      telefone_normalizado: TELEFONE,
      alteracoes: {
        periodo: { acao: 'remover' },
        procedimento_id: { acao: 'remover' },
        confirmacao: { acao: 'remover' },
      },
      publicar_ultimo_desfecho: { tipo: marcador },
      cas_estrito: { base_atualizado_em: BASE_DA_DECISAO },
    });

    assert.equal(cliente.estatisticas.chamadasUpdate.estado_conversa ?? 0, 0, 'nenhum UPDATE emitido');
    assert.deepEqual(resultado.campos_removidos, [], 'nada foi removido');
    assert.equal(resultado.ultimo_desfecho_publicado, false, 'nenhum marcador publicado');
    assert.equal(resultado.ultimo_desfecho_consumido, null);
    // Os campos do turno posterior seguem intactos no banco.
    assert.deepEqual(comoRegistro(tabelas.estado_conversa[0]!.dados), {
      periodo: 'manha',
      procedimento_id: 'do-turno-b',
    });
    // Timestamp DELIBERADAMENTE obsoleto: e o que faz a escrita auxiliar
    // seguinte falhar o proprio CAS e tambem nao escrever.
    assert.equal(resultado.atualizado_em, BASE_DA_DECISAO);
  });
}

test('cas_estrito: estado INTACTO -- limpeza e publicacao acontecem na MESMA transicao', async () => {
  const tabelas = criarTabelasFalsasVazias();
  const conversa = semearEstado(tabelas, { periodo: 'manha', confirmacao: 'sim' });
  const cliente = new ClienteFalso(tabelas);
  ...
  const resultado = await aplicarDados(cliente, {
    ...
    clinica_id: CLINICA_ID,
    telefone_normalizado: TELEFONE,
    alteracoes: { periodo: { acao: 'remover' }, confirmacao: { acao: 'remover' } },
    publicar_ultimo_desfecho: { tipo: 'reserva_criada' },
    cas_estrito: { base_atualizado_em: BASE_DA_DECISAO },
  });

  assert.deepEqual(resultado.campos_removidos.sort(), ['confirmacao', 'periodo']);
  assert.equal(resultado.ultimo_desfecho_publicado, true);
  assert.equal(cliente.estatisticas.chamadasUpdate.estado_conversa, 1, 'uma unica escrita');
});
